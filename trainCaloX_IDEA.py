...
df = pd.read_pickle('/lustre/research/hep/jdamgov/idea_ntpl_v1/IDEA_pi_pkl3_3D_3cm_2C_noProp_1cmBS/GNN.pi150GeV_100.pkl.gz')
...
def generator(batch_size,nfiles):
  ...
  while 1:
    ...
    try:
       ...
       np4=np.concatenate([np2.reshape(batch_size,nhits,1),
                           npZ.reshape(batch_size,nhits,1)],axis=2)
    ...
    if counter >= number_of_batches:
        counter = 0
        df = pd.read_pickle('/lustre/research/hep/jdamgov/idea_ntpl_v1/IDEA_pi_pkl3_3D_3cm_2C_noProp_1cmBS/GNN.pi150GeV_'+str(fcnt)+'.pkl.gz')
        df["Label"]=df["Label"]/1000.
        samples_per_file = len(df)
        number_of_batches = int(samples_per_file/batch_size)
...
input_shapes = {'points': (nhits, 3), 'features': (nhits, 2), 'mask': (nhits, 1)}
...
model_name = 'GNN_3D_3cm_CC_Zpos_50ps_noProp_R1p49_1cmBS.loss_{val_loss:01.6f}.e{epoch:03d}.h5'

...
def generator(batch_size,nfiles):
  ...
  while 1:
    ...
    try:
       ...
       npZ = np.array(df["Points"].to_list(),dtype="float32")[batch_size*counter:batch_size*(counter+1),:,2]
       np4=np.concatenate([np2.reshape(batch_size,3600,1),
                           npZ.reshape(batch_size,3600,1)],axis=2)
...
input_shapes = {'points': (3600, 3), 'features': (3600, 2), 'mask': (3600, 1)}
...
model_name = 'GNN_3D_3cm_CC_Zpos_50ps_noProp_1cmBS.loss_{val_loss:01.6f}.e{epoch:03d}.h5'

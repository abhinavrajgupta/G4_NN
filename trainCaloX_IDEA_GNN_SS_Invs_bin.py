...
def generator(batch_size,nfiles):
  ...
  df = pd.read_pickle('/lustre/research/hep/jdamgov/idea_ntpl_v1/IDEA_pi_pkl3_3D_SS_noSci_noProp_1cmBS/GNN.pi150GeV_100.pkl.gz')
  ...
  while 1:
    ...
    try:
       ...
       np3=np.concatenate([np1.reshape(batch_size,4300,1),
                           np2.reshape(batch_size,4300,1)],axis=2)
       ...
       X_batch = {'points':np.array(df["Points"][batch_size*counter:batch_size*(counter+1)].to_list(),dtype="float32"),
                ...
                'features':np1a,
                'mask':np.array(df["Mask"][batch_size*counter:batch_size*(counter+1)].to_list(),dtype="int16").reshape(batch_size,4300,1)}
    ...
    if counter >= number_of_batches:
        counter = 0
        df = pd.read_pickle('/lustre/research/hep/jdamgov/idea_ntpl_v1/IDEA_pi_pkl3_3D_SS_noSci_noProp_1cmBS/GNN.pi150GeV_'+str(fcnt)+'.pkl.gz')
        df["Label"]=df["Label"]/1000.
        df["Sum"]=np.array(df["Features"].to_list(),dtype="int32")[:,:,:1].sum(axis=-1).sum(axis=-1)
        df["Sum"]=df["Sum"]/920.
        df["Label"]=df["Label"]-df["Sum"]
...
input_shapes = {'points': (4300, 3), 'features': (4300, 1), 'mask': (4300, 1)}
...
model_name = 'GNN_3D_3cm_SS_50ps_Invs_noProp_1cmBS.loss_{val_loss:01.6f}.e{epoch:03d}.h5'

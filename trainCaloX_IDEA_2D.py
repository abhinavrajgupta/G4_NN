...
def generator(batch_size,nfiles):
  ...
  df = pd.read_pickle('/data/idea_ntpl_v1/IDEA_pi_pkl3_2D_10mm_50ps_NoSciProp/GNN.pi150GeV_100.pkl.gz')
  ...
  while 1:
    ...
    try:
       np1 = np.array(df["Features"].to_list(),dtype="int16")[batch_size*counter:batch_size*(counter+1),:,:100].sum(axis=-1)
       np2 = np.array(df["Features"].to_list(),dtype="int16")[batch_size*counter:batch_size*(counter+1),:,100:].sum(axis=-1)
       np3=np.concatenate([np1.reshape(batch_size,1400,1),
                           np2.reshape(batch_size,1400,1)],axis=2)
       ...
       X_batch = {'points':np.array(df["Points"][batch_size*counter:batch_size*(counter+1)].to_list(),dtype="float32"),
                ...
                'features':np3,
                'mask':np.array(df["Mask"][batch_size*counter:batch_size*(counter+1)].to_list(),dtype="int16").reshape(batch_size,1400,1)}
    ...
    if counter >= number_of_batches:
        counter = 0
        df = pd.read_pickle('/data/idea_ntpl_v1/IDEA_pi_pkl3_2D_10mm_50ps_NoSciProp/GNN.pi150GeV_'+str(fcnt)+'.pkl.gz')
        df["Label"]=df["Label"]/1000.
        samples_per_file = len(df)
        number_of_batches = int(samples_per_file/batch_size)
...
input_shapes = {'points': (1400, 2), 'features': (1400, 2), 'mask': (1400, 1)}
...
model_name = 'GNN_3D_1cm_50ps_NoSciProp_SC.loss_{val_loss:01.6f}.e{epoch:03d}.h5'
